...
function cargarNuevo() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var htmlTags = '<tr id="tr' + id + '">' +
                '<td>' + id + '</td>' +
                '<td id="tablaDesc' + id + '">' + descript + '</td>' +
                '<td>' +
                '<button class="btn btn-danger" onclick="DeleteUser(' + id + ')">B</button>' +
                '<button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="EditUser(' + id + ')">E</button>' +
                '</td>' +
                '</tr>';
...
function cargarUsNuevo() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#tablaDesc').html(contendor + '<tr><td>' + $('#desc').val() + '</td></tr>');
            var htmlTags = '<tr id="trUser' + id + '">' +
                '<td>' + id + '</td>' +
                '<td id="tablaiduser' + id + '">' + resultado.UsNombre + '</td>' +
                '<td id="tablapassuser' + id + '">' + resultado.UsPass + '</td>' +
                '<td id="tablamailuser' + id + '">' + resultado.UsMail + '</td>' +
                '<td id="tablaestadouser' + id + '">' + fecha + '</td>' +
                '<td>' +
                '<button class="btn btn-danger" onclick="DeleteUser(' + id + ')">' +
                '<i class="bi bi-trash"></i>'
                + '</button>' +
                '<button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="EditUser(' + id + ')">' +
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                resultado.forEach(function (persona, index) {
                    ...
                    var htmlTags = '<tr id="trUser' + id + '">' +
                        '<td>' + id + '</td>' +
                        '<td id="tablaiduser' + id + '">' + persona.nombre + '</td>' +
                        '<td id="tablapassuser' + id + '">' + persona.UsPass + '</td>' +
                        '<td id="tablamailuser' + id + '">' + persona.UsMail + '</td>' +
                        '<td id="tablaestadouser' + id + '">' + fecha + '</td>' +
                        '<td>' +
                        '<button class="btn btn-danger" onclick="borraus(' + id + ')">' +
                        '<i class="bi bi-trash"></i>'
                        + '</button>' +
                        '<button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#modalUser" onclick="abrirmodaleditarUser(' + id + ')">' +
                        '<i class="bi bi-pencil"></i>' +
                        '</button>' +
                        '</td>' +
                        '</tr>';
...
function cargarrolNuevo() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var htmlTags = '<tr id="trRol' + id + '">' +
                '<td id="tablaidrol' + id + '">' + id + '</td>' +
                '<td id="tabladescrol' + id + '">' + rodescripcion + '</td>' +
                '<td>' +
                '<button class="btn btn-danger" onclick="borrarol(' + id + ')">' +
                '<i class="bi bi-trash"></i>'
                + '</button>' +
                '<button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="abrirmodaleditarRol(' + id + ')">' +
                '<i class="bi bi-pencil"></i>' +
                '</button>' +
                '</td>' +
                '</tr>';
...
function insertarUsuarioRol(id) {
    ...
    $('input[type=checkbox]:checked').each(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                resultado = JSON.parse(result);
                $("#tabladescuseRol" + id).html($("#tabladescuseRol" + id).html()+' '+resultado.rol);
                Swal.fire({
                    position: 'top-end',
                    type: 'success',
    ...
    $("#fupForm").on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (resultado.prodes == 1) { valor1 = 'habilitado'; valor2 = 'deshabilitado'; } else { valor1 = 'deshabilitado'; valor2 = 'habilitado'; }
                $('#modalprod').modal('hide');
                var contendor = $("#tbodyprod").html();
                $('#tablaDesc').html(contendor + '<tr><td>' + $('#desc').val() + '</td></tr>');
                var htmlTags = '<tr id="trprod' + id + '">' +
                    '<td>' + id + '</td>' +
                    '<td id="tablaautorprod' + id + '">' + resultado.pronombre + '</td>' +
                    '<td id="tablaautorprod' + id + '">' + resultado.proautor + '</td>' +
                    '<td id="tablastockprod' + id + '">' + resultado.procantstock + '</td>' +
                    '<td id="tablaeditorialprod' + id + '">' + resultado.proeditorial + '</td>' +
                    '<td id="tablahabilitadoprod' + id + '">' + '<select class="form-select form-select-sm" aria-label="" onchange="cambiarEstadoProducto(1,' + id + ')">' +
                    '<option selected="">' + valor1 + '</option>' +
                    '<option value="0">' + valor2 + '</option>' +
                    '</select> </td>' +
                    '<td>' +
                    '<button class="btn btn-danger" onclick="borraProd(\'' + id + '\')">' +
                    '<i class="bi bi-trash"></i>'
                    + '</button>' +
                    '<button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#modalprod2" onclick="abrirmodaleditarprod(\'' + id + '\')">' +
                    '<i class="bi bi-pencil"></i>' +
                    '</button>' +
                    '</td>' +
                    '</tr>';
...
function cambiarRol(idrol) {
    $.ajax({
        ...
        success: function (result) {
            ...
            setTimeout(function () { // hago un setTimeout para que veas los otros pasos en tu pantalla
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                var htmlTags =
                    ...
                    '<td id="filaidmenu' + id + '">' + id + '</td>' +
                    '<td id="filanombremenu' + id + '">' + resultado.menombre + '</td>' +
                    '<td id="filaidpadremenu' + id + '">' + resultado.idpadre + '</td>' +
                    '<td id="filalinkmenu' + id + '">' + resultado.link + '</td>' +
                    '<td id="filaestadomenu' + id + '">' + estado + '</td>' +
                    '<td id="filadescripcionmenu' + id + '">' + resultado.descripcion + '</td>' +
                    '<td>' +
